...
| | |
|---|---|
| **Client** | Restaurant Les Délices d'Afrique — Akwa, Douala, Cameroun |
| **Type de projet** | Site vitrine + commande via WhatsApp (pas de panier/paiement en ligne) |
...
| **Statut** | Fonctionnel, type-check OK, build web statique OK. Contenu réel (photos, coordonnées) en attente côté client — voir §9 |
...
aucun backend (voir §7 — formulaire de contact = lien WhatsApp généré
côté client, sans serveur).
...
│   ├── services.tsx                     # Route "/services"         → Sur place / Livraison / Traiteur
...
├── components/
│   ├── layout/
│   │   ├── Header.tsx              # Nav desktop + menu mobile plein écran (Modal)
...
│   │   └── PlaceholderImage.tsx     # ⭐ Bloc visuel temporaire tant que les vraies photos manquent
│   │
...
│   │   ├── MenuItemCard.tsx         # Carte plat (image, prix, badges, bouton "Commander")
│   │   └── MenuSection.tsx          # Regroupe les MenuItemCard par catégorie
│   │
│   ├── services/
│   │   └── ServiceCard.tsx          # Carte service (réutilisée accueil + page /services)
│   │
│   ├── gallery/
...
│   └── ForceFocusedProvider.tsx     # ⚠️ Contournement d'un bug expo-router — voir §8.1, NE PAS SUPPRIMER
│
...
- **Aucun state manager externe** (pas de Redux/Zustand) : l'état est local
  ...
  de panier ni de session utilisateur
...
`/nouvelle-page`. Voir §10.1 pour le modèle exact à suivre.
...
CONFIRMER`** dans les commentaires du fichier — voir §9 pour la liste
...
réelles ne sont pas encore intégrées (voir §9) — chaque item affiche
...
## 8. SEO — implémentation & piège technique important

### 8.1 ⚠️ Bug expo-router 57.x corrigé — à connaître avant de toucher au layout
...
### 8.2 `components/seo/PageHead.tsx`
...
### 8.3 `components/seo/StructuredData.tsx`
...
### 8.5 `public/robots.txt` et `public/sitemap.xml`
...
### 8.6 Export statique — pourquoi c'est important
...
## 9. Informations manquantes à obtenir du client avant mise en ligne
...
## 10. Guides pratiques pour reprendre le projet

### 10.1 Ajouter une nouvelle page
...
### 10.2 Modifier une couleur du design
...
### 10.3 Ajouter/modifier un plat

Éditer `data/menu.ts` → tableau `menuItems`. Aucune autre modification
nécessaire (voir §7.1).

### 10.4 Composant qui a besoin d'un rendu différent web/natif
...
## 11. Ce qui n'est PAS encore implémenté (limites connues)

- **Pas de panier ni paiement en ligne** — toute commande passe par un lien
...
## 12. Vérifications effectuées avant livraison
...
## 13. Contact / paternité du code

Site conçu et développé par **KN WEB & TECHNOLOGY**, à partir du cahier des
charges fourni par le client et du menu complet transmis.
